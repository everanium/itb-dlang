...
`libitb3.so` at compile time (`-litb3` with an rpath onto the dist
...
The convenience driver builds `libitb3.so`, the binding library, and
...
`-L-L<repo>/dist/linux-amd64 -L-litb3` plus
`-L-rpath=<repo>/dist/linux-amd64` so the produced binaries resolve
`libitb3.so` without `LD_LIBRARY_PATH`. Out-of-repo deployments relink
against their own libitb3 location or ship the library on the default
loader path.
...
a Pipeline. `saveF` / `loadF` perform the file access inside libitb3.
...
```d
setMemoryLimit(4L * 1024 * 1024 * 1024);
setGCPercent(100);
```
...
The harness builds `libitb3.so` + the binding, compiles every
...
libitb3 global state). Test binaries carry `-unittest` and run with
...
- Stream sessions borrow their Pipeline without a compiler-enforced
  ...
  Pipeline's lifetime.
- libitb3 must be reachable at runtime through the rpath baked in at
...
- **Loading the D binding disables druntime's parallel GC marking
  by default** (via a `pragma(crt_constructor)` in `itb3.pipeline`
  that sets `gc:parallel:0` before runtime-option parsing). Without
  this, the druntime helper threads contend with libitb3's Go
