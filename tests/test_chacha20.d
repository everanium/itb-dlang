// ChaCha20-focused D binding coverage.
//
// Mirrors bindings/rust/tests/test_chacha20.rs one-to-one. ChaCha20
// ships at a single width (-256). Each Rust `#[test] fn` becomes a
// `void test*()` here, called from `main()`; the per-test inner
// loops over (nonce, primitive, mac) are inlined directly via
// `foreach`.
...
private static immutable HashRow[] CHACHA20_HASHES = [
