...
//
// Symmetric counterpart to bindings/python/tests/test_aescmac.py applied
// to the high-level itb.Encryptor surface. AES-CMAC ships only at -128
// (the AES block size).
//
// Mirrors bindings/rust/tests/test_easy_aescmac.rs one-to-one. Each
// Rust `#[test] fn` becomes a `void test*()` here, called from main();
...
// (hash, ITB_seed_width) — AES-CMAC ships only at -128 (AES block size).
private static immutable HashRow[] AESCMAC_HASHES = [
    HashRow("aescmac", 128),
];

// PRF-key length (bytes) per primitive — locks in the FFI-surfaced
...
void main()
{
    ...
    testRoundtripSizes();
    writefln("test_easy_aescmac: ALL OK");
}
