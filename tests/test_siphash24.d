// SipHash-2-4-focused D binding coverage.
//
// W128 counterpart to test_aescmac.d. SipHash ships at a single width
// (-128) and has no internal fixed key — its keying material is the
// per-call seed components — so `expectedKeyLen("siphash24")` is 0
// and the persistence path stores empty bytes for the hash key.
//
// Mirrors bindings/rust/tests/test_siphash24.rs one-to-one. Each Rust
...
// SipHash has no internal fixed key (keyed by seed components), so
// the FFI hash_key field is empty.
        ...
        foreach (keyBits; validKeyBits)
        {
            foreach (n; NONCE_SIZES)
            {
                ...
                ulong[] ssComps; ubyte[] ssKey;
                {
                    ...
                    nsKey = ns.hashKey.dup;   // empty for SipHash
                    ...
                    assert(nsKey.length == expectedKeyLen(h.name),
                        "hashKey length mismatch (siphash24 expects 0)");
                    assert(cast(int) nsComps.length * 64 == keyBits,
                        "components length mismatch");

                    ciphertext = encrypt(ns, ds, ss, plaintext);
                }
                // Empty hashKey is the deterministic-rebuild path for
                // SipHash-2-4 — the ITB FFI accepts the empty slice as
                // the explicit "no internal fixed key" signal.
...
void main()
{
    ...
    testRoundtripSizes();
    writefln("test_siphash24: ALL OK");
}
